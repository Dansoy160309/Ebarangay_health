...
@section('title', 'Disposed Vaccine Batches')
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 sm:gap-8">
            <div class="max-w-xl">
                ...
                <h1 class="text-2xl sm:text-4xl font-black text-gray-900 tracking-tight leading-tight mb-2 sm:mb-3">
                    Vaccine Disposal <span class="text-red-600 underline decoration-red-200 decoration-4 underline-offset-4">Log</span>
                </h1>
                <p class="text-gray-500 font-medium text-xs sm:text-sm leading-relaxed">
                    Complete history of disposed expired vaccine batches.
                </p>
            </div>
        ...
        <form method="GET" class="flex flex-wrap items-center gap-3">
            ...
            <select name="vaccine_id"
                ...
                @foreach($vaccines as $vaccine)
                    <option value="{{ $vaccine->id }}" @selected(request('vaccine_id') == $vaccine->id)>
                        {{ $vaccine->name }}
                    </option>
                @endforeach
            </select>
            ...
            @if(request()->hasAny(['date_from', 'date_to', 'vaccine_id']))
                <a href="{{ route('admin.vaccines.disposals') }}" class="p-2.5 bg-gray-100 text-gray-500 rounded-xl hover:bg-gray-200 transition-all">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        ...
                        <th class="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-[0.15em]">Vaccine</th>
                        ...
                        <th class="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-[0.15em]">Disposed Qty</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($disposals as $row)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-8 py-6 text-sm font-black text-gray-900">
                                {{ $row->vaccine?->name ?? 'Unknown vaccine' }}
                            </td>
                            <td class="px-8 py-6">
                            ...
                            <td class="px-8 py-6 text-sm font-black text-red-600">{{ $row->expiry_date?->format('M d, Y') ?? 'N/A' }}</td>
                            <td class="px-8 py-6 text-sm font-black text-gray-900">{{ number_format($row->disposed_quantity ?? 0) }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-8 py-16 text-center text-gray-500 font-semibold">No disposed vaccine batches found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
