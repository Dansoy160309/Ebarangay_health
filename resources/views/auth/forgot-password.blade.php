...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gray-50 relative overflow-hidden">
    ...
    <div class="max-w-sm w-full bg-white rounded-[1.5rem] shadow-2xl p-6 md:p-7 relative z-10 transition-all duration-300 hover:shadow-3xl border border-gray-100">
        <div class="text-center mb-6">
            ...
            <h2 class="text-xl font-black text-gray-900 mb-1 tracking-tight">Forgot Password?</h2>
            <p class="text-[10px] text-gray-500 font-medium leading-relaxed">No worries! Enter your email and we'll send you a reset link.</p>
        </div>

        {{-- Localized alerts removed to keep it clean as requested --}}

        <form method="POST" action="{{ route('password.forgot.submit') }}" class="space-y-5">
            @csrf

            <div>
                <label for="email" class="block text-[10px] font-black text-gray-700 uppercase tracking-widest ml-1 mb-1.5">Email Address</label>
                <div class="relative group">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus
                        class="block w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-brand-100 focus:border-brand-500 focus:bg-white text-gray-900 placeholder-gray-400 transition-all font-bold text-sm"
