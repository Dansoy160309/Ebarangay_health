    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <div class="flex -space-x-2">
                <div class="w-8 h-8 rounded-full bg-brand-100 border-2 border-white flex items-center justify-center text-brand-600 text-[10px] font-bold">A</div>
                <div class="w-8 h-8 rounded-full bg-purple-100 border-2 border-white flex items-center justify-center text-purple-600 text-[10px] font-bold">B</div>
                <div class="w-8 h-8 rounded-full bg-emerald-100 border-2 border-white flex items-center justify-center text-emerald-600 text-[10px] font-bold">C</div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <div class="space-y-1">
                            ...
                            <p class="text-sm font-bold text-gray-700 leading-relaxed">
                                @if($announcement->expires_at)
                                    This advisory remains active and valid until <span class="text-brand-600">{{ $announcement->expires_at->format('F d, Y') }}</span>. 
                                @endif
        ...
        <div class="space-y-8">
            ...
            <div class="bg-gray-900 rounded-[3rem] p-8 text-white shadow-xl relative overflow-hidden group">
                ...
                <a href="{{ route('patient.dashboard') }}" class="block w-full py-4 bg-white/10 hover:bg-white/20 border border-white/10 rounded-2xl text-center text-xs font-black uppercase tracking-[0.2em] transition-all">
                    Contact Center
                </a>
            </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        @foreach($recent as $item)
                            <a href="{{ route('patient.announcements.show', $item) }}" class="block p-6 bg-white border border-gray-100 rounded-[2rem] shadow-sm hover:shadow-lg hover:border-brand-200 transition-all group">
